...
describe Merchant do
  ...
  describe 'instance methods' do
    it 'revenue' do
      expect(@merchant1.revenue).to eq(26.0)
    end
  end

  describe 'class methods' do
    it 'top_merchants_revenue' do
      expect(Merchant.top_merchants_revenue(2).first.id).to eq(@merchant1.id)
      expect(Merchant.top_merchants_revenue(2).second.id).to eq(@merchant3.id)
    end

    it 'top_merchant_items' do
      expect(Merchant.top_merchants_items(2).first.id).to eq(@merchant3.id)
      expect(Merchant.top_merchants_items(2).second.id).to eq(@merchant1.id)
    end
  end
end
